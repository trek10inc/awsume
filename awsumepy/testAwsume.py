...
class TestInput(unittest.TestCase):
    def test_handle_parameters(self):
        #normal use-cases
        self.assertTrue(awsumepy.handle_parameters(['-d', '-r', '-s']).refresh)
        self.assertTrue(awsumepy.handle_parameters(['-d', '-r', '-s']).show)
        self.assertTrue(awsumepy.handle_parameters(['-d', '-r', '-s']).default)
        self.assertEqual(awsumepy.handle_parameters(['-d']).profile_name, 'default')
        self.assertEqual(awsumepy.handle_parameters(['input_name']).profile_name, 'input_name')
        #possibly misleading inputs
        self.assertEqual(awsumepy.handle_parameters([]).profile_name, 'default')
        #unrecognized inputs will stop the script
        with self.assertRaises(SystemExit):
            awsumepy.handle_parameters(['one_name', 'two_name'])

    def test_valid_mfa_token(self):
        #normal use-cases
        self.assertTrue(awsumepy.valid_mfa_token('123456'))
        #incorrect mfa tokens
        self.assertFalse(awsumepy.valid_mfa_token(''))
        self.assertFalse(awsumepy.valid_mfa_token('12345'))
        self.assertFalse(awsumepy.valid_mfa_token('1234567'))
        self.assertFalse(awsumepy.valid_mfa_token('abcdef'))
        self.assertFalse(awsumepy.valid_mfa_token('abc123'))
        self.assertFalse(awsumepy.valid_mfa_token('a23456'))
        self.assertFalse(awsumepy.valid_mfa_token('12345*'))
...
class TestINISectionMethods(unittest.TestCase):
    ...
    def test_get_sections(self):
        #normal use-cases
        self.assertEqual(awsumepy.get_sections('./test/example'), self.expected_example_sections)
        #getting sections from an empty file
        self.assertEqual(awsumepy.get_sections('./test/emptyFile'), collections.OrderedDict())
        #getting sections from a file that doesn't exist
        with self.assertRaises(SystemExit):
            awsumepy.get_sections('./test/a/path/that/does/not/exist')
...
class TestProfileMethods(unittest.TestCase):
    ...
    def test_is_role(self):
        #normal use-cases
        self.assertTrue(awsumepy.is_role(self.example_role_profile))
        self.assertFalse(awsumepy.is_role(self.expected_source_profile))
...
class TestSessionMethods(unittest.TestCase):
    ...
    def test_is_valid(self):
        #normal use-cases
        self.assertTrue(awsumepy.is_valid(self.expected_valid_session))
        #testing an empty sessin
        self.assertFalse(awsumepy.is_valid(self.empty_session))
        self.assertFalse(awsumepy.is_valid(self.expired_session))
        self.assertFalse(awsumepy.is_valid(self.invalid_session))
    ...
    def test_read_session(self):
        #normal use-cases
        self.assertEqual(awsumepy.read_session("./test/", "exampleSession"), self.expected_valid_session)
